...
Holographic VFX is an open-source project (`v1.0.1`). Because commercial code-signing certificates cost hundreds of dollars annually, binaries are unsigned. Windows SmartScreen displays a warning on any newly downloaded unsigned executable until it establishes global download reputation.
